...
describe Cart do
  ...
  describe '#push_to_cart' do
    it 'returns the price of all elements' do
      ...
      expect(test1.carts[client_id_1]['price']).to eql([2, 3, 4, 5, 6])
    end
    it 'cannot push to two different carts at once' do
      expect{ test1.push_to_cart(client_id_1, client_id_2, 'test', 'fail') }.to raise_error(ArgumentError)
    end
  end
  describe '#name_to_string' do
    it 'return the names as string without []' do
      expect(test1.name_to_string(client_id_1)).to eql('apple, banana, strawberry, orange, watermelon')
    end
    it 'cannot transform two different carts into a single string' do
      expect{ test1.name_to_string(client_id_1, client_id_2) }.to raise_error(ArgumentError)
    end
  end
  describe '#cart_total' do
    it 'sums the values on the cart' do
      expect(test1.cart_total(client_id_1)).to eql(20)
    end
    it 'only handles integers correctly' do
      expect(test2.cart_total(client_id_2)).not_to eql(14)
    end
    it 'does not sum two different carts' do
      expect{ test1.cart_total(client_id_1, client_id_2) }.to raise_error(ArgumentError)
    end
  end
end
